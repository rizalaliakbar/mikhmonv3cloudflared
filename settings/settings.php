...
if (!isset($_SESSION["mikhmon"])) {
  ...
  if ($id == "settings" && explode("-",$router)[0] == "new") {
    ...
    fwrite($f, "\n'$'data['".$router."'] = array ('1'=>'".$router."!','".$router."@|@','".$router."#|#','".$router."%','".$router."^','".$router."&Rp','".$router."*10','".$router."(1','".$router.")','".$router."=10','".$router."@!@disable','".$router."|@|no','".$router."|#|','".$router."|%|');");
    fclose($f);
    $search = "'$'data";
    $replace = (string)"$data";
  ...
  if (isset($_POST['save'])) {
    ...
    $scloudflared = ($_POST['cloudflared']);
    $scloudflaredhost = ($_POST['cloudflaredhost']);
    $scloudflaredforward = ($_POST['cloudflaredforward']);


    $search = array('1' => "$session!$iphost", "$session@|@$userhost", "$session#|#$passwdhost", "$session%$hotspotname", "$session^$dnsname", "$session&$currency", "$session*$areload", "$session($iface", "$session)$infolp", "$session=$idleto", "'$session'", "$session@!@$livereport", "$session|@|$cloudflared", "$session|#|$cloudflaredhost", "$session|%|$cloudflaredforward");

    $replace = array('1' => "$sesname!$siphost", "$sesname@|@$suserhost", "$sesname#|#$spasswdhost", "$sesname%$shotspotname", "$sesname^$sdnsname", "$sesname&$scurrency", "$sesname*$sreload", "$sesname($siface", "$sesname)$sinfolp", "$sesname=$sidleto", "'$sesname'", "$sesname@!@$slivereport", "$sesname|@|$scloudflared", "$sesname|#|$scloudflaredhost", "$sesname|%|$scloudflaredforward");
    	   ...
    	   <div class="row">
            ...
            <div class="col-12">
             ...
             <div class="col-12">
                 <div class="card">
                     <div class="card-header">
                         <h3 class="card-title">Connect use Cloudflared Tunnel</h3>
                     </div>
                     <div class="card-body">
                         <?php
                         if(@exec('echo EXEC') == 'EXEC'){
                             $exec = '<strong style="color:#00be77;">[OK]</strong> Exec Ready !';
                             $exec_state = true;
                         } else {
                             $exec = '<strong style="color:#ff0000;">[ERR]</strong> Exec Not Enabled !';
                             $exec_state = false;
                         }
                         $cmd_output=null;
                         $cmd_status=null;
                         exec('cloudflared --version', $cmd_output, $cmd_status);
                         if ($cmd_status == 0) {
                             $cloudflareds = '<strong style="color:#00be77;">[OK]</strong> ' . $cmd_output[0];
                             $cloudflared_state = true;
                         } else {
                             $cloudflareds = '<strong style="color:#ff0000;">[ERR]</strong> Cloudflared Not Installed !';
                             $cloudflared_state = false;
                         }
                         ?>
                         <table style="width:100%;border: 1px solid;border-collapse: collapse;">
                             <tr style="border-collapse: collapse;">
                                 <th style="border-collapse: collapse;border: 1px solid;padding:5px;width:130px;">Exec Function</th>
                                 <td style="border-collapse: collapse;border: 1px solid;padding:5px;"><?= $exec ?></td>
                             </tr>
                             <tr style="border-collapse: collapse;border: 1px solid;">
                                 <th style="border-collapse: collapse;border: 1px solid;padding:5px;width:130px;">Cloudflared</th>
                                 <td style="border-collapse: collapse;border: 1px solid;padding:5px;"><?= $cloudflareds ?></td>
                             </tr>
                         </table>
                         <table class="table">
                             <tr>
                                 <td>Use Cloudflared ?</td>
                                 <td>
                                     <select class="form-control" name="cloudflared" id="cloudflared" <?php if (!$exec_state || !$cloudflared_state) { echo 'disabled'; } ?>>
                                         <?php if (!$exec_state || !$cloudflared_state) { echo '<option value="no">disabled</option>'; } ?>
                                         <?php if (!empty($cloudflared)) { ?>
                             ...
                             <tr>
                                 <td>Hostname</td>
                                 <td><input class="form-control" id="cloudflaredhost" type="text" name="cloudflaredhost" title="Cloudflared Hostname" value="<?= $cloudflaredhost ?>" <?php if (!$exec_state || !$cloudflared_state) { echo 'disabled'; } ?> /></td>
                             </tr>
                             <tr>
                                 <td>Forward to</td>
                                 <td><input class="form-control" id="cloudflaredforward" type="text" name="cloudflaredforward" title="Forward to localhost" value="<?= $cloudflaredforward; ?>" <?php if (!$exec_state || !$cloudflared_state) { echo 'disabled'; } ?> /></td>
                             </tr>
                         </table>
                     </div>
                 </div>
            ...
            <div class="col-12">
				...
				<table class="table table-sm">
					<tr>
	  					<td class="align-middle">IP MikroTik </td><td><input class="form-control" type="text" size="15" id="ipmik" name="ipmik" title="IP MikroTik / IP Cloud MikroTik" value="<?= $iphost; ?>" required="1"/></td>
					</tr>
					<tr>
...
const cloudflared = document.getElementById('cloudflared')
const cloudflaredForward = document.getElementById('cloudflaredforward')
const ipmik = document.getElementById('ipmik')
const cloudflaredhost = document.getElementById('cloudflaredhost')
cloudflared.addEventListener('change', function () {
    if (cloudflared.value == 'yes') {
        cloudflaredhost.disabled = false
        cloudflaredForward.disabled = false
        ipmik.value = cloudflaredForward.value
        cloudflaredForward.addEventListener('keyup', function () {
            ipmik.value = cloudflaredForward.value
        })
    } else {
        cloudflaredhost.disabled = true
        cloudflaredForward.disabled = true
    }
})
